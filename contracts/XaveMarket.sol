...
contract XaveMarket is AccessControlEnumerable, ERC1155Holder {
    ...
    function buyToken(
        ...
        address _currency
    ) external onlyIfNftExists(_nft) onlyIfCurrencyExists(_currency) {
        ...
        erc20.safeTransferFrom(sender, address(this), totalAmount);
    ...
    function getPurchaseKey(
        address nft,
        uint256 tokenId
    ) external view onlyIfNftExists(nft) returns (PurchaseKey memory) {
    ...
    function removeValidCurrency(
        address currency
    ) external onlyRole(NFT_ADMIN) onlyIfCurrencyExists(currency) {
        require(
    ...
    function removeAllDefaultPrices(
        address nft
    ) external onlyRole(NFT_ADMIN) onlyIfNftExists(nft) {
        address[] memory emptyCurrencies;
        _removeDefaultPrice(nft, emptyCurrencies, true);
    ...
    function setMaxToken(
        uint256 _maxTokenPurchase
    ) external onlyRole(NFT_ADMIN) {
        require(_maxTokenPurchase <= 25, "Cannot go over 25");
    ...
    function withdraw() external onlyRole(WITHDRAW) {
        ...
        for (uint256 i = 0; i < _validCurrenciesSet.length(); i++) {
            ...
            if (bal > 0) {
                currency.safeTransfer(sender, bal);
                if (!withdrawn) {
                    withdrawn = true;
                }
            }
